...
<pre><code>await fetch("/api/start-flow", { method: "POST", headers: { "x-api-key": "${exampleKey}", "Content-Type": "application/json" }, body: JSON.stringify({ system: "CRM Onboarding", orderNumber: "ORD-12345", description: "New account setup" }) });</code></pre>
    ...
    try {
      ...
      if (targetUserId === currentUser.id && req.body.role && req.body.role !== targetUser.role) {
        return res.status(403).json({ 
          message: "You cannot change your own role. Another admin must change it." 
        });
      }
      
      // SECURITY: Prevent users from changing their own status
      if (targetUserId === currentUser.id && req.body.status && req.body.status !== targetUser.status) {
        return res.status(403).json({ 
      ...
      if (req.body.role && req.body.role !== targetUser.role) {
        await storage.logRoleChange({
          userId: targetUserId,
          changedBy: currentUser.id,
          organizationId: currentUser.organizationId,
          oldRole: targetUser.role,
          newRole: req.body.role,
          reason: req.body.roleChangeReason || 'Admin role change',
        });
        console.log(`🔐 Role change: ${targetUser.email} from ${targetUser.role} to ${req.body.role} by ${currentUser.email}`);
      }
      
      // Prevent changing sensitive fields via this endpoint
      const { organizationId, id, createdAt, invitedBy, registrationType, ...allowedUpdates } = req.body;
      
      const user = await storage.updateUserDetails(targetUserId, allowedUpdates);
      res.json(user);
    } catch (error) {
  ...
  app.put("/api/users/:id/status", isAuthenticated, requireAdmin, addUserToRequest, async (req: any, res) => {
    try {
      ...
      // SECURITY: Prevent users from changing their own status (self-deactivation)
      if (targetUserId === currentUser.id && newStatus !== targetUser.status) {
        return res.status(400).json({ 
          message: "You cannot change your own account status. Contact another administrator." 
        });
      }
  ...
  app.post("/api/invitations", isAuthenticated, requireAdmin, addUserToRequest, async (req: any, res) => {
    try {
      const currentUser = req.currentUser;
      ...
      const hasPending = pendingInvitations.some((inv: any) => inv.status === 'pending');
      if (hasPending) {
        return res.status(400).json({ 
      ...
      console.log(`📧 Invitation created for ${email}, token: ${invitationToken}`);
  ...
  app.get("/api/invitations", isAuthenticated, requireAdmin, addUserToRequest, async (req: any, res) => {
    try {
      const currentUser = req.currentUser;
  ...
  app.get("/api/invitations/:token", async (req: any, res) => {
    try {
      ...
      // Get organization and admin details
      ...
      res.json({
        ...
        invitedByName: admin ? `${admin.firstName} ${admin.lastName}` : 'Unknown',
  ...
  app.post("/api/invitations/:token/accept", async (req: any, res) => {
    try {
      ...
      const user = await storage.createUser({
        email: invitation.email,
        firstName: invitation.firstName || displayName?.split(' ')[0] || '',
        lastName: invitation.lastName || displayName?.split(' ').slice(1).join(' ') || '',
      ...
      // Create user-admin relationship
      await storage.createUserAdminRelationship({
        userId: user.id,
  ...
  app.post("/api/invitations/:token/reject", async (req: any, res) => {
    try {
      const { token } = req.params;
      const { reason } = req.body;
      ...
      await storage.rejectInvitation(token, reason);
      
      console.log(`❌ User rejected invitation: ${invitation.email}`);
  ...
  app.delete("/api/invitations/:id", isAuthenticated, requireAdmin, addUserToRequest, async (req: any, res) => {
    try {
      const currentUser = req.currentUser;
      const { id } = req.params;
      
      // Get invitation to verify ownership
      const invitation = await storage.getInvitationByToken(id);
      if (invitation && invitation.organizationId !== currentUser.organizationId) {
        return res.status(403).json({ message: "Access denied" });
      }
      
      await storage.cancelInvitation(id);
  ...
  app.post("/api/invitations/:id/resend", isAuthenticated, requireAdmin, addUserToRequest, async (req: any, res) => {
    try {
      const currentUser = req.currentUser;
      const { id } = req.params;
      ...
      const updatedInvitation = await storage.updateInvitationStatus(id, 'pending');
